...
// Eliminar importación de SplitText ya que es un plugin de pago y probablemente no esté disponible para usuarios normales sin clave de licencia.
// Recurriremos a animaciones de texto estándar o GSAP simple si es necesario, o verificaremos si el usuario tiene licencia.
// Asumiendo que el usuario "Flip" podría no tener licencia GSAP Club. Usando GSAP estándar para una implementación segura.
...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    buf[0] = sigmoid(buf[0]);
    // Force Violet #842DB4 (0.518, 0.176, 0.706)
    float intensity = max(buf[0].x, max(buf[0].y, buf[0].z));
    return vec4(intensity * 0.518, intensity * 0.176, intensity * 0.706, 1.0);
  }
  
  void main() {
    vec2 uv = vUv * 9.0 - 4.5; uv.y *= -1.0;
...
// Agregar interfaz para el elemento personalizado de intellisense JSX
declare module '@react-three/fiber' {
    interface ThreeElements {
        cPPNShaderMaterial: ThreeElement<typeof CPPNShaderMaterial> & { side?: THREE.Side };
    }
}
...
export default function NeuralNetworkHero({
    ...
    useGSAP(
        () => {
            // Lógica de SplitText reemplazada con simple desvanecimiento hacia arriba debido a posibles problemas de licencia
            // Si el usuario confirma que tiene licencia club podemos volver a agregar SplitText
            ...
            if (microRef.current) {
                tl.to(microRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }, '-=0.2');
            }
        },
        { scope: sectionRef },
    );
