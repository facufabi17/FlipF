...
const CPPNShaderMaterialCian = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1) },
  vertexShader,
  fragmentShader
);

extend({ CPPNShaderMaterialCian });

declare module '@react-three/fiber' {
  interface ThreeElements {
    cPPNShaderMaterialCian: ThreeElement<typeof CPPNShaderMaterialCian> & { side?: THREE.Side };
  }
}
...
function ShaderPlane() {
  ...
  return (
    <mesh ref={meshRef} position={[0, -0.75, -0.5]}>
      ...
      <cPPNShaderMaterialCian ref={materialRef} side={THREE.DoubleSide} />
    </mesh>
  );
}
